def run():
    ...
    if is_allow_all_feature:
        print(
            f"--- すべての処理（リネーム：{entry_rename_target} -> {entry_rename_src} + フォルダ移動：{entry_move_dir} + 指定したファイル：{entry_file_select}）"
        )
    elif is_allow_rename and is_allow_move_dir:
        print(
            f"--- 特定処理（リネーム：{entry_rename_target} -> {entry_rename_src} + フォルダ移動：{entry_move_dir}）"
        )
    elif is_allow_rename and is_allow_file_select:
        print(
            f"--- 特定処理（リネーム：{entry_rename_target} -> {entry_rename_src} + 指定したファイル：{entry_file_select}）"
        )
    elif is_allow_rename:
        print(f"--- 特定処理（リネーム：{entry_rename_target} -> {entry_rename_src}）")
    else:
        print("リネーム名を入力してください")
